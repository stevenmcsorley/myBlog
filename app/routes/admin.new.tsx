...
export const action: ActionFunction = async ({ request }) => {
  ...
  await prisma.post.create({
    data: { title, slug, excerpt, content, authorId: userId },
  });
...
export default function NewPost() {
  ...
  const { GROQ_API_KEY } = useLoaderData<{ GROQ_API_KEY: string }>();

  const [title, setTitle] = useState(actionData?.fields?.title || "");
  const [slug, setSlug] = useState(actionData?.fields?.slug || "");
  const [excerpt, setExcerpt] = useState(actionData?.fields?.excerpt || "");
